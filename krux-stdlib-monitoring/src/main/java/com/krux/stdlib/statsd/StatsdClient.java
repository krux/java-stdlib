...
public class StatsdClient {
    ...
    public StatsdClient( InetAddress host, int port, Logger logger, int queueSize ) throws SocketException {
        this(host, port, logger, queueSize, detectCharset());
    }

    public StatsdClient( InetAddress host, int port, Logger logger, int queueSize, Charset charset ) throws SocketException {
        if ( host == null ) {
    ...
    protected void handleError( String message, String stat, Exception e ) {
        if ( logger != null && logger.isErrorEnabled() ) {
            logger.error( "{}: sending {} to {}", message, stat, toString(), e );
        }
    }
}
